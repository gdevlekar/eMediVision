    ...
    public partial class frmVoucherEntry : Form
    {
        ...
        public   string GetMarquee()
        {
            ...
            using (conn)
            {
                ...
                command.CommandText = "sp_GetEarlyExpiraryMedicines";
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (selectedRow.Cells[0].Value != null)
                if (Int32.TryParse(selectedRow.Cells[0].Value.ToString(), out v))
                {
                    ...
                    SQL = "select   DrugName,Packing,Comp,Shelf,BatchNo,ExpiraryDate,MRP,Qty,Amount from products where ID =" + selectedRow.Cells[0].Value;
                ...
                else
                {
                    ...
                    SQL = "select   DrugName,Packing,Comp,Shelf,BatchNo,ExpiraryDate,MRP,Qty,Amount from products where DrugName ='" + selectedRow.Cells[0].Value + "'";
            ...
            //--------------------------
            decimal TotalQty = 0, Amount = 0;
            foreach (DataGridViewRow Row in dataGridView1.Rows)
            {
                if (Row.Cells["Qty"].Value != null)
                {
                    TotalQty = TotalQty + Convert.ToInt16(Convert.ToString(Row.Cells["Qty"].Value)=="" ? "0" : Row.Cells["Qty"].Value);
                    Amount = Amount + Convert.ToDecimal(Convert.ToString(Row.Cells["Amount"].Value) == "" ? "0" : Row.Cells["Amount"].Value);
                }

            }

            lblTotalBigQty.Text = TotalQty.ToString() ;
            lblTotalPrice.Text = Amount.ToString("##,##,###.00");
            //--------------------------
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Delete)
            {
                ...
                //    if (Row.Cells["Qty"].Value != null)
                //    {
                //        TotalQty = TotalQty + Convert.ToInt16(Row.Cells["Qty"].Value);
                //        Amount = Amount + Convert.ToDecimal(Row.Cells["Amount"].Value);
                //    }
